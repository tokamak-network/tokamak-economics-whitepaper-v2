...
\textbf{Rule 3 (Bridged TON as Performance Metric).} L2 performance is measured by its Bridged TON amount. We define $B_i$ as the amount of TON bridged to L2 $i$, and $T_i$ as the amount of TON staked by the L2 sequencer on the L1 contract.
...
The total seigniorage allocated to the L2 ecosystem is determined using the hyperbolic saturation function \cite{michaelis1913kinetik}. This function imposes diminishing returns as performance increases, thereby preventing oversized L2s from capturing disproportionate rewards. The total L2 seigniorage $y(x)$ is defined as:
\begin{equation}
